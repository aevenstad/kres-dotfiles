export APPTAINER_CACHEDIR="/bigdata/Jessin/Softwares/.apptainer"
export NXF_APPTAINER_CACHEDIR="/bigdata/Jessin/Softwares/containers"
export NXF_SINGULARITY_CACHEDIR="/bigdata/Jessin/Softwares/containers"

export KRAKEN_DEFAULT_DB="/bigdata/Jessin/Softwares/Database/minikraken_20171013_4GB/"
export KRAKEN2_DEFAULT_DB="/bigdata/Jessin/Softwares/Database/minikraken2_v1_8GB/"
export CENTRIFUGE_DEFAULT_DB="/bigdata/Jessin/Softwares/Database/centrifuge-db/p_compressed+h+v"

path_prepend_if_dir "/usr/local/go/bin"
path_prepend_if_dir "/bigdata/Jessin/Scripts/illumina_kres/scripts"
path_prepend_if_dir "/bigdata/Jessin/Scripts/wrapper_scripts"
path_prepend_if_dir "/bigdata/Jessin/Softwares/containers"
path_prepend_if_dir "$HOME/.local/bin"
path_append_if_dir "/opt/nvim-linux-x86_64/bin"
export PATH

if command -v keychain >/dev/null 2>&1; then
  eval "$(keychain --quiet --eval storesøster kres_128_github 2>/dev/null)"
fi
